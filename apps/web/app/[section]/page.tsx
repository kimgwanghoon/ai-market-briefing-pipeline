...
import { dateValue, getBriefings, Kind, labels, Query } from '@/lib/data';
...
export default async function Section({ params, searchParams }: { params: Promise<{section:string}>; searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  const current = !archive ? result.rows[0] : undefined;
  const [prior, peer] = current ? await Promise.all([
    getBriefings({kind:section, before:current.generated_at},1),
    section === 'weekly' ? Promise.resolve(null) : getBriefings({kind:section === 'live' ? 'daily' : 'live', before:current.generated_at},1),
  ]) : [null,null];
